...
from .functions import (
    allowed_file,
    allowed_preview_file,
    get_unique_filepath,
    generate_temporary_link,
    get_rating_counts,
    get_chunk,
    get_unique_filepath_preview,
...
def upload_video(current_user, session):  # current_user is passed from decorator
    ...
    try:
        ...
        if preview_file:
            if allowed_preview_file(
                preview_file.filename
            ):  # Proceed only if file has correct extension
                # Save preview
                preview_filename = secure_filename(preview_file.filename)
                preview_path = os.path.join(
                    app.config["PREVIEW_FOLDER"], preview_filename
                )
    ...
    try:
                ...
                if tags:
                    try:
                        ...
                        return (
                            jsonify(
                                {
                                    "message": "Invalid tag format. Tags must be integers."
                                }
                            ),
                            400,
                        )
                    ...
                    for tag_id in tags_list:
                        ...
                        if not tag:
                            return (
                                jsonify({"message": f"Tag with ID {tag_id} not found"}),
                                400,
                            )
        ...
        if "preview" in request.files:
            ...
            if preview_file and allowed_preview_file(preview_file.filename):
                ...
                if (
                    video.IdMediaPreview
                    and video.IdMediaPreview != 1
                    and video.IdMediaPreview != 2
                ):  # Check if video has preview
                    old_preview = (
                        session.query(MediaPreview)
                        .filter_by(IdMediaPreview=video.IdMediaPreview)
                        .first()
                    )
                    if old_preview:
                        try:
...
def get_video_link(user, session, id):
    ...
    try:
        ...
        # View History Logic
        view_history_entry = (
            session.query(ViewHistory).filter_by(IdUser=user.IdUser, IdMedia=id).first()
        )
        if view_history_entry:
            ...
            new_view_history = ViewHistory(
                IdUser=user.IdUser, IdMedia=id, ViewTime=datetime.datetime.now()
            )
        ...
        total_views = (
            session.query(func.sum(ViewHistory.ViewCount))
            .filter(ViewHistory.IdMedia == id)
            .scalar()
        )
        total_views = (
            int(total_views) if total_views is not None else 0
        )  # Explicitly convert to int

        # Get unique viewer count
        unique_viewers = (
            session.query(func.count(distinct(ViewHistory.IdUser)))
            .filter(ViewHistory.IdMedia == id)
            .scalar()
        )
        unique_viewers = (
        ...
        tags = [
            {"id": tag.IdTag, "name": tag.TagName} for tag in media.tags
        ]  # Get tags
        ...
        if media.preview:
            ...
            if os.path.exists(preview_path):
                return send_from_directory(
                    os.path.dirname(preview_path), os.path.basename(preview_path)
                )
            else:
...
def rate_video(current_user, session, id):
    ...
    try:
        ...
        existing_rating = (
            ...
            .filter_by(IdUser=current_user.IdUser, IdMedia=id)
            .first()
        )
        ...
        if rating_value == 0:  # Remove rating
            if existing_rating:
                ...
                session.commit()
                likes, dislikes, user_rating_value = get_rating_counts(
                    session, id, current_user.IdUser
                )
                return (
                    jsonify(
                        {
                ...
                return (
                    jsonify({"message": "No rating to remove"}),
                    200,
                )  # Nothing to remove
        ...
        if rating_value == 1:
            rating_type = (
                session.query(RatingTypes).filter_by(NameRating="Like").first()
            )
        elif rating_value == -1:
            rating_type = (
                session.query(RatingTypes).filter_by(NameRating="Dislike").first()
            )
...
def stream_video_from_link(link_id, filename):
    ...
    try:
        ...
        resp = Response(
            chunk, 206, content_type=content_type, direct_passthrough=True
            ...
            video_list.append(
                {
                    ...
                    "upload_time": (
                        video.UploadTime.isoformat() if video.UploadTime else None
                    ),
                    "tags": tags,
...
def get_video_recommendations(user, session):
    ...
    try:
        recent_videos = (
            ...
            .filter(
                and_(
                    ...
                    ViewHistory.IdUser == user.IdUser,
                )
            )
            .order_by(ViewHistory.ViewTime.desc())
            .all()
        )
        num_recent_videos = len(recent_videos)

        recommended_videos = recommendation_generator(
            user, session, num_recent_videos, recent_videos
        )
        ...
        recommended_audios = recommendation_generator(
            user, session, num_recent_audios, recent_audios, is_audio=True
        )
        ...
        return jsonify({"video": video_list, "audio": audio_list}), 200

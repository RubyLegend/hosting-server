...
def get_company_info(current_user, session, id):
    ...
    try:
        ...
        # Count subscribers
        subscriber_count = (
            session.query(func.count(Subscribers.IdSubscriber))
            .filter(Subscribers.IdCompany == id)
            .scalar()
            or 0
        )
        is_subscribed = False
        existing_subscription = (
            session.query(Subscribers)
            .filter_by(IdUser=current_user.IdUser, IdCompany=id)
            .first()
        )
    ...
    try:
        ...
        if "logo" in request.files:
            ...
            if image_file and allowed_logo_file(image_file.filename):
                ...
                if (
                    company.companyLogo.LogoPath
                    and company.companyLogo.IdCompanyLogo != 1
                    ...
                    try:
                        ...
                        app.logger.exception(
                            f"Error while removing old company logo: {e}"
                        )
...
def delete_company(user, session, id):
    ...
    try:
        ...
        session.commit()
        return (
            jsonify({"message": "Company and associated data deleted successfully"}),
            200,
        )
...
def get_company_owners(user, session, id):
    ...
    try:
        ...
        owners = [
            {"user_id": x.users.IdUser, "Email": x.users.Email}
            for x in users_in_company
            if x.access_levels == owner_level
        ]
...
def update_company_owners(user, session, id):
    ...
    try:
        ...
        if not isinstance(data, object):
            return (
                jsonify({"message": "Invalid request data. Expected a JSON object."}),
                400,
            )
        ...
        user_role = (
            session.query(UserRoles).filter_by(IdUser=user_id, IdCompany=id).first()
        )

        if not user_role:
            new_user_role = UserRoles(
                IdUser=user_id, IdCompany=id, IdAccessLevel=owner_level.IdAccessLevel
            )
            ...
            return (
                jsonify(
                    {
                        "message": "Cannot change this user rights. Remove existing ones before modifying."
                    }
                ),
                400,
            )
...
def delete_company_owners(user, session, id):
    ...
    try:
        ...
        if not isinstance(data, list):
            return (
                jsonify({"message": "Invalid request data. Expected a JSON array."}),
                400,
            )
        ...
        for item in data:
            user_role = (
                session.query(UserRoles).filter_by(IdUser=item, IdCompany=id).first()
            )
            ...
            if user_role and user_role.access_levels == owner_level:
                ...
                return (
                    jsonify({"message": "User does not have that role. No changes."}),
                    200,
                )
...
def get_company_moderators(user, session, id):
    ...
    try:
        ...
        moderators = [
            {"user_id": x.users.IdUser, "Email": x.users.Email}
            for x in users_in_company
            if x.access_levels == moderator_level
        ]
...
def update_company_moderators(user, session, id):
    ...
    try:
        ...
        if user.IdUser == user_id:
            return (
                jsonify("Cannot modify yourself, otherwise you will lose access."),
                403,
            )
        ...
        if not user_role:
            new_user_role = UserRoles(
                IdUser=user_id,
                IdCompany=id,
                IdAccessLevel=moderator_level.IdAccessLevel,
            )
            session.add(new_user_role)
        elif user_role.IdAccessLevel == user_level.IdAccessLevel:

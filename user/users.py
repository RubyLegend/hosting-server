...
def search_users(user, session):
    ...
    try:
        users = (
            session.query(Users).filter(Users.Email.like(f"%{search_string}%")).all()
        )
...
def get_user_info(user, session, id):
    ...
    try:
        ...
        user_info = {  # Construct the user info dictionary
            ...
            "birthday": (
                user_to_get.Birthday.isoformat() if user_to_get.Birthday else None
            ),
        }
...
"""
    try:
        ...
        user_to_update.Birthday = data.get(
            "birthday", user_to_update.Birthday.isoformat()
        )
        ...
        am_i_admin = user_has_access_level(
            user, get_access_level_by_name(session, "Admin"), session
        )

        if old_password and new_password and not am_i_admin:
            if bcrypt.checkpw(
                old_password.encode("utf-8"), user_to_update.Password.encode("utf-8")
            ):
                user_to_update.Password = bcrypt.hashpw(

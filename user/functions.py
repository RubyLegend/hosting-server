...
def generate_token(app, redis_client, user_id):
    ...
    payload = {
        ...
        + datetime.timedelta(
            minutes=int(app.config["TOKEN_TIMEOUT"])
    ...
    redis_client.setex(
        ...
        datetime.timedelta(minutes=int(app.config["TOKEN_TIMEOUT"])),
        token,
    )
            ...
            try:
                ...
                if user_id_bytes:
                    try:
                        ...
                        user = (
                            session.query(Users).filter_by(IdUser=user_id).first()
...
def get_access_level_by_name(session, access_name):
    access_level_record = (
        session.query(AccessLevels).filter_by(AccessName=access_name).first()
    )
    return access_level_record if access_level_record else None


def user_has_access_level(
    user, required_level, session, weak_comparison=True, company_id=None
):
    ...
    for role in user.user_roles:  # Iterate through user's roles
        if role.access_levels == required_level:
            ...
            weak_comparison
            and role.access_levels.AccessLevel >= required_level.AccessLevel
        ):
            if company_id is not None:
                if role.IdCompany == company_id:
                    return True
...
def admin_level(f):
    ...
    def decorated_function(user, session, *args, **kwargs):
        try:
            if not user_has_access_level(
                user, get_access_level_by_name(session, "Admin"), session
            ):
...
def user_or_admin_required(f):
    ...
    def decorated_function(user, session, *args, **kwargs):
        ...
        if user.IdUser != user_id_to_update and not user_has_access_level(
            user, get_access_level_by_name(session, "Admin"), session
        ):
            return (
                jsonify(
                    {
                        "message": "Forbidden. You can only access your own data or request for admin access."
                    }
                ),
                403,
            )
...
def moderator_level(f):
    ...
    def decorated_function(user, session, *args, **kwargs):
        try:
            if not user_has_access_level(
                user, get_access_level_by_name(session, "Moderator"), session
            ):
    ...
    def decorated_function(user, session, *args, **kwargs):  # Now accepts user object
        ...
        for role in user.user_roles:  # Use user.user_roles
            if role.access_levels.AccessName == "Admin":
                return f(user, session, *args, **kwargs)
            if (
                role.IdCompany == company_id
                and role.access_levels.AccessName == "Company Owner"
            ):
                return f(user, session, *args, **kwargs)
...
def has_company_owner_access(user, session):
    ...
    return user_has_access_level(
        user, get_access_level_by_name(session, "Company Owner"), session
    )
